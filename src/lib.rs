...
use gui::GuiRenderer;
use rand::Rng;
use wgpu::{include_wgsl, Texture, TextureUsages};
use winit::{
...
impl App {
    ...
    async fn set_window(&mut self, window: Window) {
        ...
        let renderer = Renderer::new(&window, &self.instance, surface).await;
...
impl ApplicationHandler for App {
    ...
    fn window_event(
        ...
        renderer.gui.handle_input(window, &event);
    }
}
...
impl Renderer {
    async fn new(window: &Window, instance: &wgpu::Instance, surface: wgpu::Surface<'static>) -> Renderer {
        ...
        let graphics_shader = device.create_shader_module(include_wgsl!("conway_renderer.wgsl"));
        ...
        let compute_shader = device.create_shader_module(wgpu::include_wgsl!("conway_compute.wgsl"));
    ...
    fn render(&mut self, window: &Window) -> Result<(), wgpu::SurfaceError> {
        ...
        self.gui.draw(window, &self.device, &self.queue, &self.config, gui::gui, &mut encoder, &view);
